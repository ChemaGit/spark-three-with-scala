...
object LogParser {
  ...
  def main(args: Array[String]) {
    ...
    // Extract the user-agent field from each log line
    val userAgent = lines.map(x => {
      val matcher:Matcher = pattern.matcher(x)
    ...
    val userAgentCount = userAgent.map(x => (x, 1)).reduceByKeyAndWindow(_ + _, _ - _, Seconds(300), Seconds(1))
    ...
    val sortedUserAgent = userAgentCount.transform(rdd => rdd.sortBy(t => t._2, false))
    sortedResults.print()
    sortedUserAgent.print()
